...
function ProductsUpdatePage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [categories, setCategories] = useState<ICategory[]>([]);

  const [initialValues, setInitialValues] = useState<IProductUpdate>({
    title: "",
    description: "",
    price: 0,
    images: [],
  ...
  const handleSubmit = async (values: IProductUpdate) => {
          ...
          {({
            ...
            <Form className="m-5">
              ...
              <SelectGroup
                ...
                optionKey="id"
                optionLabel="name"
                initialItemId={values.categoryId}
              ></SelectGroup>
              <div className="mt-5">
                <button
                  type="submit"
